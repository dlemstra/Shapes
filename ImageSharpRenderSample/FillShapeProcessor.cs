...
namespace ImageSharpRenderSample
{
        ...
        where TColor : struct, IPackedPixel, IEquatable<TColor>
    {
        ...
        {
            var shaperRect = this.poly.Bounds;
            var b = new RectangleF(shaperRect.X, shaperRect.Y, shaperRect.Size.Width, shaperRect.Size.Height);

            Rectangle rect = RectangleF.Ceiling(b); // rounds the points out away from the center
            ...
            ArrayPool<Shaper2D.Point> arrayPool = ArrayPool<Shaper2D.Point>.Shared;
            ...
            using (BrushApplicator<TColor> applicator = this.fillColor.CreateApplicator(sourcePixels, rect))
            {
                ...
                y =>
                {
                    Shaper2D.Point[] buffer = arrayPool.Rent(maxIntersections);
                    ...
                    x =>
                    {
                        ...
                        try
                        {
                            ...
                            for (int y = minY; y < maxY; y++)
                            {
                                ...
                                if (opacity > float.Epsilon && opacity < 1)
                                {
                                    Vector4 backgroundVector = sourcePixels[x, y].ToVector4();
                                    Vector4 sourceVector = applicator.GetColor(currentPoint).ToVector4();
        ...
        private static void Swap(Shaper2D.Point[] data, int left, int right)
        {
        ...
        private static void QuickSortY(Shaper2D.Point[] data, int size)
        {
            int hi = Math.Min(data.Length - 1, size - 1);
            QuickSortY(data, 0, hi);
        }

        private static void QuickSortY(Shaper2D.Point[] data, int lo, int hi)
        {
            if (lo < hi)
            {
        ...
        private static void QuickSortX(Shaper2D.Point[] data, int size)
        {
            int hi = Math.Min(data.Length - 1, size - 1);
            QuickSortX(data, 0, hi);
        }

        private static void QuickSortX(Shaper2D.Point[] data, int lo, int hi)
        {
            if (lo < hi)
            {
        ...
        private static int PartitionX(Shaper2D.Point[] data, int lo, int hi)
        {
        ...
        private static int PartitionY(Shaper2D.Point[] data, int lo, int hi)
        {
